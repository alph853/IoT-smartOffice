...
class ThingsboardClient(MqttCloudClientRepository):
    ...
    def connect_device(self, device_name: str):
        return self.client.gw_connect_device(device_name)

    def disconnect_device(self, device_name):
        return self.client.gw_disconnect_device(device_name)
    ...
    def send_rpc_reply(self, request_id: str, response: dict, device: str | None = None, qos: int = 1):
        ...
        self.client.gw_send_rpc_reply(device, request_id, response, qos)
    ...
    def _handle_rpc_task_done(self, task):
        try:
            task.result()
            logger.info(f"RPC has been published successfully")
    ...
    async def _handle_rpc_async(self, request_id, content):
        ...
        try:
            if method == "deleteDevice":
                ...
                event = GatewayDeviceDeletedEvent(**params)
